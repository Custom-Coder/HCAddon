package io.github.custom_coder.hcaddon.config
...
object HcaddonClientConfig {
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            HcaddonServerConfig.holder.config = HcaddonServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            HcaddonServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = Hcaddon.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use HcaddonServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = HcaddonServerConfig.ServerConfig()
    }
