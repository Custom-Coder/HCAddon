package io.github.custom_coder.hcaddon.registry
...
typealias RegistrarEntry<T> = HcaddonRegistrar<T>.Entry<out T>

abstract class HcaddonRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(Hcaddon.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is HcaddonRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
